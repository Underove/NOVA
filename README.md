...
```mermaid
flowchart TD
    Web["🌐 웹 (Next.js)"] -->|REST / WebSocket| API
    iOS["📱 iOS 앱 (SwiftUI)"] -->|REST / WebSocket| API
    API["⚙️ FastAPI 백엔드 · Railway<br/>function calling · pgvector RAG"] --> Data
    Data["📊 KIS · KRX · DART · 네이버 · 웹 검색<br/>🤖 OpenAI / Gemini &nbsp;·&nbsp; 🗄️ PostgreSQL &nbsp;·&nbsp; 🔔 APNs"]
```
...
| **투자 정보 팩트체크** | 리포트·뉴스·지라시를 DART 공시·뉴스와 교차검증(pgvector RAG), 신뢰도 🟢/🟡/🔴 신호등 |
...
> 🛠️ **AI 어시스턴트가 호출하는 도구** — `get_stock_price` · `get_portfolio` · `search_recent_news` · `get_technical_indicators` · `get_dart_disclosures` · `screen_stocks`
...
**백엔드**
- FastAPI (Python 3.12)
- LLM: OpenAI gpt-5.4 (일반 호출은 mini, 브리핑은 full 하이브리드) + Google Gemini — function calling(tool use) 기반
